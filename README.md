...
La arquitectura se basa en una estructura modular que separa las vistas (`pages`) de los componentes reutilizables (`components`). Se utiliza **redirección automática** hacia el Login para usuarios no autenticados y **React Router** para asegurar que la jerarquía de vistas sea clara, escalable y mantenga una alta densidad de información sin saturar al usuario.
